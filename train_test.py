...
CFG = {
    ...
    'lr': 1e-5 / 2 , # large 7.5e-5  base 1e-5 / 2
...
handler = logging.FileHandler(f"logs/garbage_meta_{CFG['model_arch']}_train_cls.log")
...
class MetaModel(nn.Module):
    def __init__(self, model_arch, feature_dim, meta_feature_dim, num_classes) -> None:
        super().__init__()
        self.backbone = timm.create_model(model_arch, num_classes=CFG['class_num'], pretrained=False)
    ...
    def forward(self, x, meta_feature):
        ...
        features = torch.cat((res, meta_feature), dim=-1)
        # print(features.shape)   #[8,1041]
        # outs = self.dropout(features)
...
if __name__ == '__main__':
    ...
    temp_model = timm.create_model(CFG['model_arch'], num_classes=CFG['class_num'], pretrained=False)
    ...
    for epoch in range(CFG['epochs']):
        ...
        with torch.no_grad():
            ...
            if (temp_train_acc > beat_train_acc):
                torch.save(model.state_dict(), './checkpoints/garbage_imgsize_{}_{}_mixtype_{}_mixprob_{}_seed_{}_ls_{}_epochs_{}_diffLR_{}_meta.pth'.format(
                                                CFG['img_size'],
                                                CFG['model_arch'],
                                                CFG['mix_type'],
                                                CFG['mix_prob'],
